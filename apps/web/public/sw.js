...
const SHELL = ['/', '/app', '/manifest.webmanifest', '/favicon.svg', '/icon-192.svg', '/icon-512.svg'];
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    caches.match(req).then((cached) => {
      const fetchPromise = fetch(req).then((r) => { caches.open(CACHE).then((c) => c.put(req, r.clone())); return r; }).catch(() => cached);
