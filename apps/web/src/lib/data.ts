 ...
 * EmroozData built on an anon-key client. It's safe for public reads (recipes,
 * cuisines, ingredients) because RLS is authoritative. Private per-user reads
 * from server components should use the auth-aware helper in
...
export function getData(): EmroozData {
  ...
  if (!cachedSupabase) {
    ...
    cachedSupabase = new SupabaseEmroozData(createClient(url, anon));
  }
